...
def events(request, album_id):
    album_selected = Picture.objects.filter(photo_album=album_id).select_related('photo_album', 'event_name').distinct('event_name').order_by("event_name","id")
...
def event(request, event_id):
    event_selected = Picture.objects.filter(event_name=event_id).select_related('event_name', 'photo_album').order_by("id")
    event_name = get_list_or_404(Events, pk=event_id)[:1]
    context = {"event_selected": event_selected, "event_name": event_name}

    return render(request, 'albums/event.html', context)